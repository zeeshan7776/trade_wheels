...
const HeroSectionBox = ({ car }) => {
  ...
  const iconDescription = [
    ...
    `${car.mileage.min} - ${car.mileage.max}`,
    `${car.fuelType}`,
    `${car.transmission}`,
  ];
    ...
    >
      ...
      >
        <div className="flex md:flex-col lg:flex-row lg:items-center md:gap-2 mt-4 md:mt-0 text-red-800 text-xs md:text-md">
          <div className="font-thin mr-1">PKR</div>
          <div className="font-bold text-red-700">
            {car.price.min} - {car.price.max}
          </div>
          <div className="font-thin text-xs ml-1">(*Ex-Factory Price)</div>
        </div>
        <div>
          <img src={process.env.PUBLIC_URL + car.image} alt="" />
        </div>
      </div>
