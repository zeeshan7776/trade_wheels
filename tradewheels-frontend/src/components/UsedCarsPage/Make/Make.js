...
const Make = ({ autoSlide = false, autoSlideInterval = 3000 }) => {
  ...
  return (
    <div className="w-full px-2 sm:px-8 text-dark-text">
      {/* Heading */}
      <motion.div
      ...
      <div className="w-full my-14 sm:my-24">
        <div>
          <motion.div
            className="flex items-center p-5 gap-x-8 sm:gap-x-10 overflow-hidden"
          ...
          >
            {manufacturers.map((manufacturer, index) => (
              ...
              >
                <div
                  className="hover:text-dark-white flex items-center transition-transform ease-out duration-500 bg-dark-bkg 
                  min-w-24 min-h-28 flex-col justify-center rounded-full"
                  style={{ transform: `translateX(-${curr * 141.5}%)` }}
                  key={index}
                >
                  <div className="w-full h-14 flex items-center justify-center">
                    <img src={manufacturer.logo} alt={manufacturer.name}></img>
                  </div>
